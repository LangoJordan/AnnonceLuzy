...
export default function AdEdit({ user = {}, ad = null, features = [], spaces = [], countries = [], cities = [], categories = [] }) {
  const { theme } = useThemeStore();
  const [step, setStep] = useState(1);
  const [mainPhotoPreview, setMainPhotoPreview] = useState(ad?.main_photo || null);
  ...
  const handleAddFeature = () => {
    if (!featureLabel.trim() || !featurePhotoUrl.trim()) {
      setFieldErrors(prev => ({
        ...prev,
        feature: 'Le label et l\'URL sont requis'
      }));
      return;
    }

    if (!isValidUrl(featurePhotoUrl)) {
      setFieldErrors(prev => ({
        ...prev,
        feature: 'L\'URL n\'est pas valide'
      }));
      return;
    }
    ...
    const newFeature = {
      label: featureLabel,
      photo: featurePhotoUrl,
    };

    const updatedFeatures = [...allFeatures, newFeature];
  ...
  const removeFeature = (index) => {
    const newFeatures = allFeatures.filter((_, i) => i !== index);
  ...
  const handleSubmit = (e) => {
    ...
    patch(route('agency.ad-update', ad.id), {
      onSuccess: () => {
        router.visit(route('agency.ad-details', ad.id));
      },
    });
  };
          ...
          <form onSubmit={handleSubmit} className="max-w-4xl mx-auto px-8 py-12">
            ...
            {recentlySuccessful && (
              <div className={`mb-6 p-4 rounded-lg border flex items-start gap-3 ${theme === 'dark' ? 'border-emerald-500/20 bg-emerald-500/10' : 'border-emerald-300 bg-emerald-50'}`}>
                ...
                <p className={theme === 'dark' ? 'text-emerald-300' : 'text-emerald-700'}>✓ Annonce mise à jour avec succès!</p>
              </div>
            )}
            ...
            {step === 3 && (
              <div>
                ...
                <div className={`rounded-lg border p-6 mb-8 ${theme === 'dark' ? 'border-slate-700 bg-slate-900' : 'border-gray-200 bg-white'}`}>
                  ...
                  <div>
                    <label className={`block text-sm font-bold mb-3 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-900'}`}>
                      Photos de facettes (max 5)
                    </label>
                    <div className="space-y-3 mb-4">
                      <div className="grid grid-cols-2 gap-3">
                        <input
                          type="text"
                          placeholder="Label de la facette (ex: Caractéristique)"
                          value={featureLabel}
                        ...
                        <input
                          type="url"
                          placeholder="https://exemple.com/facette.jpg"
                          value={featurePhotoUrl}
                          onChange={(e) => setFeaturePhotoUrl(e.target.value)}
                          className={`px-4 py-3 rounded-lg border focus:outline-none focus:ring-2 focus:ring-purple-500 ${
                            theme === 'dark' ? 'bg-slate-800 border-slate-700 text-gray-100' : 'bg-white border-gray-300 text-gray-900'
                          }`}
                        />
                      ...
                      {fieldErrors.feature && <p className={`text-sm ${theme === 'dark' ? 'text-red-400' : 'text-red-600'}`}>{fieldErrors.feature}</p>}
                    </div>

                    {allFeatures.length > 0 && (
                      <div className="mt-6">
                        <p className={`text-sm font-bold mb-3 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-900'}`}>
                          Facettes ({allFeatures.length}/5):
                        </p>
                        <div className="grid grid-cols-5 gap-3">
                          {allFeatures.map((preview, index) => (
                            <div key={index} className="relative group">
                              ...
                              <div className={`absolute inset-0 bg-black/50 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 transition`}>
                                <p className="text-xs text-white font-bold text-center px-1">{preview.label}</p>
                              </div>
                              <button
                                type="button"
